// Best-effort status mirror across the three lifecycle views of a unit.
// Each mirror call is wrapped in try/catch by the caller — primary writes
// must never fail because the secondary mirror failed.
//
// Join keys used to walk between the three tables:
//   available_units.deal_id          → crm_deals.id
//   available_units.tenancy_unit_id  → tenancy_schedule_units.id
...
import { codeToLeasingStatus, leasingStatusToCode, codeMatchesLeasingStatus } from "@shared/lease-status-mirror";
...
export async function mirrorFromLeasingSchedule(leasingUnitId: string, newStatus: string, ctx: Ctx): Promise<void> {
  ...
  const au = auRow.rows[0];
  if (!au) return; // schedule-only unit, nothing to mirror
