...
export async function detectTile(sharp: any, planImage: Buffer, W: number, H: number, ox: number, oy: number, fw: number, fh: number, known: string[], overview = false, regions: import("./plan-unit-detection").PlanUnitRegion[] = [], focused = false): Promise<import("./plan-unit-detection").DetectedPlanUnit[]> {
