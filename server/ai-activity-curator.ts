...
export function buildActivityQuestion(subject: ActivitySubject): string {
  ...
  return [
    ...
    `  • search_calendar — for meetings, viewings, calls and diary entries (matches subject/body/attendees/location)`,
    ``,
    `Pick smart distinctive terms — name, brand, address, contact emails, postcode. Don't just search the literal subject string. Run multiple queries with different terms if the first returns weak results.`,
    ``,
