...
Live from Xero (15-min cache, Refresh button forces a pull): income/gross/opex/net FYTD, monthly P&L chart, full P&L lines, cash by bank account, aged debtors (Debtors card = Xero outstanding + the pre-Xero Sage legacy line), paid-this-FY matched to deals, WIP pipeline projection (NEG 50% / SOL 75% / EXC 90% weights + completed-not-invoiced), commission statements, card spend, **Costs & forecast** (cost run rate from last 3 full months, projected FY costs, projected FY net, top cost lines with % share, biggest movers vs 3-month average, cash due in vs out by month, largest open bills, recurring commitments). Front and centre sits the **Company outlook** panel: projected FY income (Xero billed to date + weighted deal book + legacy Sage line), the cost base split into basic company costs vs salaries & payroll with commissions computed live from the deal boards' fee splits through the tier bands, average cost per month (the breakeven billing line), next-6-months and FY profit, per-partner profit share on top of the £145k basic, and a month-by-month chart overlaying the last few years' Sage billings. Below it: the **Cashflow forecast** board (Wendy's editable cost plan + the typed Legacy Sage receivables line), **Historical billings** (FY2019–26 by team/agent/client/company) and the **Equity partners** remuneration board (salary/bonus/advances per FY + the live profit-share forecast). Other staff asking for these numbers: politely say the company finance view is restricted to the equity group.
