...
Live from Xero (15-min cache, Refresh button forces a pull): income/gross/opex/net FYTD, monthly P&L chart, full P&L lines, cash by bank account, aged debtors, paid-this-FY matched to deals, WIP pipeline projection (NEG 50% / SOL 75% / EXC 90% weights + completed-not-invoiced), commission statements, card spend, **Costs & forecast** (cost run rate from last 3 full months, projected FY costs, projected FY net, top cost lines with % share, biggest movers vs 3-month average, cash due in vs out by month, largest open bills, recurring commitments). Other staff asking for these numbers: politely say the company finance view is restricted to the equity group.
...
Tenant brands with news (Google News + RSS), an Instagram board (all posts with media), signals, requirements, comps, competitor section, Compliance & KYC panel, and a designed 2-page Brand Pack PDF download (Bordeaux palette). Every tenant brand gets the full treatment — there is no "tracked" subset and the UI no longer shows a tracked label or filter (Woody, 2026-08-24). The brand list was culled in Aug 2026 to substantive brands only.

### Detail pages on the PHONE (deal / property / brand / company)
On phones these record pages show a row of section pills under the page title and ONE section at a time (desktop shows everything in two columns). Deal page pills: Overview / Brand / KYC / Activity / Files. Property page pills: Overview / Boards / Deals & units / Files & contacts / KYC / Activity. Brand/company profile pills on the phone: Chat / Contacts / Compliance / Intel — Intel holds best sellers, portfolio activity, signals, the UK stores map, the competition set and the Instagram board. (Desktop brand profile keeps its own Profile / Stores / Relationship / Intel / More sections.) If a phone user says they "can't find" the KYC panel, files, competitor set or key contacts on a record: tell them to tap the matching pill under the title.
