...
- Cashflow board (/cashflow, equity/admin only, linked from the Finance page header): monthly Budget vs Actual cash flow grid seeded from the 2026/27 forecast — click a cell to edit, totals and the closing-balance chain recompute, closing-balance chart on top, plus a Forecast-vs-Xero cross-check (cash at bank + monthly income/expenses). Behind a PASSWORD on top of the equity gate (unlock lasts the browser session; also unlockable from the Finance page's Cashflow forecast section, which shows the compact chart + low point). On phones the board shows one month at a time with prev/next arrows instead of the full grid.
