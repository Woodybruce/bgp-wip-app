...
- Cashflow forecast lives ON the Finance page (no separate board, no password — /cashflow just redirects to /finance; equity/admin only). Cash in is app-driven: pipeline-weighted deal fees + Xero invoices due, plus one editable "Legacy receivables (pre-Xero)" line; cash out is Wendy's editable costs plan (Budget/Actual per month); the closing-balance chain anchors on Xero's live cash at bank. Click cost/legacy cells to edit; add cost lines with the button. On phones it shows one month at a time with prev/next arrows.
