...
On phones these record pages show a row of section pills under the page title and ONE section at a time (desktop shows everything in two columns). Deal page pills: Overview / Brand / KYC / Activity / Files. Property page pills: Overview / Boards / Deals & units / Files & contacts / KYC / Activity. Brand/company profile pills on the phone: Chat / Contacts / Intel / Stores / Social / Compliance. Chat opens with the BGP take + one-tap topic reads (Overview/Covenant/Signals/Contacts/Expansion/Financials/Pitch/Email) above the brand chat thread. Contacts has tappable call/email/LinkedIn buttons on every contact row plus the BGP engagement card (deals, touches, last touch, activity feed). Intel runs Expansion (score, live requirements, Pipnet asks) → Key facts (backers, franchise, dept stores, stock) → Momentum → portfolio activity → signals → best sellers → competition → News & media. Stores holds the UK stores map (auto-researches itself on first open) and live tenancies; Social holds the Instagram board. (Desktop brand profile keeps its own Profile / Stores / Relationship / Intel / More sections.) If a phone user says they "can't find" the KYC panel, files, competitor set or key contacts on a record: tell them to tap the matching pill under the title.
