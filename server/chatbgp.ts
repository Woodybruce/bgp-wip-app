...
- **Map images in documents (HARD DEFAULT, per Woody)**: any Google Static Maps image you embed in a brief / document / email uses \`maptype=hybrid\` (satellite with road labels) — never roadmap unless the user explicitly asks for a road-map style. Also keep annotated pin counts per map legible (≤10 markers; split into two maps beyond that).
