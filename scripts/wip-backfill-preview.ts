/**
 * WIP backfill preview — produces two CSVs you sign off before any writes:
 *
 *   tmp/wip-properties.csv  — distinct property names extracted from active WIP
 *                             deal names, with current link status and proposed
 *                             match against crm_properties.
 *
 *   tmp/wip-landlords.csv   — distinct groupName values from active WIP deals,
 *                             with deal count, landlord-link coverage and
 *                             proposed canonical landlord (crm_companies match).
 *
 * Run with:  npx tsx scripts/wip-backfill-preview.ts
 *
...
const CLOSED_STATUSES = new Set(["Completed", "Lost", "Invoiced", "Leasing - Completed"]);
...
function extractPropertyName(dealName: string): string {
  // Most WIP deals: "Bluewater - Shake Shack", "South Molton Street – Some Brand"
...
async function main() {
  ...
  const dealsRes = await pool.query<{
    ...
    select id, name, group_name, property_id, landlord_id, status
    from crm_deals
  `);
  ...
  console.log(
    `[wip-preview] ${dealsRes.rows.length} total deals, ${activeDeals.length} active`
  );
  ...
  for (const b of [...propBuckets.values()].sort((a, b) => b.dealCount - a.dealCount)) {
    // Match strategies: exact normalised, then startsWith, then contains.
  ...
  fs.writeFileSync(path.join(outDir, "wip-properties.csv"), csv(propRows));
  console.log(
    `[wip-preview] tmp/wip-properties.csv — ${propBuckets.size} distinct properties`
  );

  // ── Landlord (groupName) bucket ────────────────────────────────────────────
  type LandBucket = {
    groupName: string;
    norm: string;
    dealCount: number;
  ...
  fs.writeFileSync(path.join(outDir, "wip-landlords.csv"), csv(landRows));
  console.log(
    `[wip-preview] tmp/wip-landlords.csv — ${landBuckets.size} distinct group names`
  );
...
main().catch((err) => {
  console.error("[wip-preview] failed:", err);
  process.exit(1);
});
