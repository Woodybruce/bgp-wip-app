import { Building2, Briefcase, BarChart3, LayoutGrid, User as UserIcon, Sparkles } from "lucide-react";
...
export type TagType = "user" | "company" | "property" | "deal" | "unit" | "contact";

export const TAG_TOKEN_SOURCE = "@\\[([^\\]]+)\\]\\(tag:(user|company|property|deal|unit|contact)\\/([^)\\s]+)\\)";
...
export function TagChip({ type, id, name }: { type: TagType; id: string; name: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[11px] font-medium align-baseline mx-0.5 ${meta.chip} ${path ? "cursor-pointer hover:opacity-80" : "cursor-default"} transition-opacity`}
      title={path ? `Open ${meta.label.toLowerCase()}: ${name}` : name}
