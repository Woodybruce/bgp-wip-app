...
function scoreBand(score: number): { label: string; color: string } {
  // Bands on the brand page's Expansion score (v2), set from the first
  // full scan (top score 56); 30 is where the "turned HOT" alert fires.
  if (score >= 30) return { label: "Very Hot", color: "text-red-600" };
  if (score >= 20) return { label: "Hot",      color: "text-orange-500" };
  if (score >= 10) return { label: "Watch",    color: "text-yellow-600" };
...
export default function BrandHunterBoard() {
  ...
  const hotCount   = brands.filter(b => b.expansionScore >= 30).length;
  const watchCount = brands.filter(b => b.expansionScore >= 10 && b.expansionScore < 30).length;
