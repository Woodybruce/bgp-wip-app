      ...
      <CardContent className="p-3 pt-0 space-y-2.5">
        {editing ? (
          ...
          <div className="w-full flex flex-col">
            ...
            {/* KYC summary strip */}
            {(c.kyc_status || c.aml_risk_level || data.kyc?.doc_count > 0 || c.kyc_expires_at) && (
              <div className="rounded-md border border-border/40 bg-muted/20 p-2">
                <div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground flex items-center gap-1 mb-1.5">
                  <ShieldCheck className="w-3 h-3" /> KYC &amp; AML
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                  <div>
                    <div className="text-[10px] text-muted-foreground">Status</div>
                    <div className={`font-semibold ${c.kyc_status === "pass" ? "text-emerald-700" : c.kyc_status === "warning" ? "text-amber-600" : c.kyc_status === "fail" ? "text-red-600" : "text-muted-foreground"}`}>
                      {c.kyc_status === "pass" ? "Passed" : c.kyc_status === "warning" ? "Review" : c.kyc_status === "fail" ? "Failed" : "Not run"}
                    </div>
                  </div>
                  {c.aml_risk_level && (
                    <div>
                      <div className="text-[10px] text-muted-foreground">AML risk</div>
                      <div className={`font-semibold ${c.aml_risk_level === "high" ? "text-red-600" : c.aml_risk_level === "medium" ? "text-amber-600" : "text-emerald-700"}`}>
                        {c.aml_risk_level}
                      </div>
                    </div>
                  )}
                  {c.aml_pep_status && (
                    <div>
                      <div className="text-[10px] text-muted-foreground">PEP</div>
                      <div className="font-semibold">{c.aml_pep_status}</div>
                    </div>
                  )}
                  {data.kyc?.doc_count > 0 && (
                    <div>
                      <div className="text-[10px] text-muted-foreground">Documents</div>
                      <div className="font-semibold">{data.kyc.doc_count}</div>
                    </div>
                  )}
                  {c.kyc_expires_at && (
                    <div>
                      <div className="text-[10px] text-muted-foreground">Expires</div>
                      <div className="font-semibold">{new Date(c.kyc_expires_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "2-digit" })}</div>
                    </div>
                  )}
                  {data.kyc?.last_uploaded_at && (
                    <div>
                      <div className="text-[10px] text-muted-foreground">Last doc</div>
                      <div className="font-semibold">{new Date(data.kyc.last_uploaded_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "2-digit" })}</div>
                    </div>
                  )}
                </div>
              </div>
