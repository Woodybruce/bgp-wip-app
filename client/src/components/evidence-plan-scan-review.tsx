...
export function EvidencePlanScanReview({ open, onOpenChange, planId, level, onSaved, onRefresh, scanRunning }: {
  ...
  const protectedSuggestion = selected && review && isPending(selected, review) && selected.suggestedUnitIds.length === 1
    ? review.existingUnits.find(unit => unit.id === selected.suggestedUnitIds[0] && protectedTarget(unit, review)
      && !review.clearedUnitIds.includes(unit.id) && isValidPolygon(unit.polygon)) || null : null;
  ...
  const comparisonUnit = inspectedOldUnit || target || (!targetId ? protectedSuggestion : null);
  ...
  const viewBox = useMemo(() => {
    const outline = inspectedOldUnit?.polygon || (selected
      ? [...selected.polygon, ...(isValidPolygon(comparisonUnit?.polygon) ? comparisonUnit!.polygon! : [])] : null);
  ...
  }, [selected, inspectedOldUnit, comparisonUnit, zoomSelected, width, height]);
  ...
  const content = <>
    ...
    {result.isLoading ? <div className="grid gap-4 md:grid-cols-2" aria-label="Loading scan review"><Skeleton className="h-64" /><div className="space-y-3"><Skeleton className="h-12" /><Skeleton className="h-44" /></div></div>
      ...
      : <>
        <p className="text-sm font-mono tabular-nums" data-testid="scan-review-summary">{review.summary.detected} detected · {review.summary.added} added · {review.summary.refined} refined · {review.summary.current} already current · {Object.keys(review.applied).length} applied in review · {pending.length} scan proposals not applied</p>
        <p className="text-sm text-muted-foreground">Unapplied proposals can overlap units already saved on the plan, including protected outlines. This count does not indicate missing units.</p>
        ...
        <div className="grid min-h-0 gap-4 md:grid-cols-[minmax(0,1.35fr)_minmax(300px,1fr)]">
          <div className="min-w-0 space-y-2">
            ...
            <svg viewBox={viewBox} role="group" aria-label="Detected boundaries on the source plan. Select a boundary to review its unit." className="w-full h-[32dvh] md:h-[49dvh] min-h-56 rounded-lg border border-border bg-muted/30" data-testid="scan-review-preview">
              ...
              {comparisonUnit && isValidPolygon(comparisonUnit.polygon) && <polygon points={points(comparisonUnit.polygon!)} fill={inspectedOldUnit ? "hsl(var(--foreground) / .12)" : "none"} stroke="hsl(var(--foreground))" strokeWidth={2} strokeDasharray="6 4" vectorEffect="non-scaling-stroke" pointerEvents="none" data-testid="scan-review-old-outline" />}
            ...
            <p className="text-[11px] text-muted-foreground">Solid outline: scan proposal. Dashed outline: saved boundary being compared. Dot: proposed label position.</p>
            {selected && <div className="rounded-lg border border-border bg-card p-3 space-y-3" data-testid="scan-review-selection">
              ...
              {protectedSuggestion && <div className="rounded-md border border-border bg-muted p-3 space-y-1" data-testid="scan-review-protected-comparison"><p className="text-sm font-semibold">Saved outline protected · compare with scan</p><p className="text-sm">{protectedSuggestion.unitRef}{protectedSuggestion.tenantName ? ` · ${protectedSuggestion.tenantName}` : ""} has a {protectedSuggestion.source === "manual" ? "manually saved" : "reviewed"} outline. It remains unchanged and cannot be replaced in this review.</p>{!targetId && <p className="text-[11px] text-muted-foreground">Its saved boundary is shown dashed alongside the scan proposal.</p>}</div>}
