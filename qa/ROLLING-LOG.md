...
### r280 · 2026-08-13 · ROUND IN PROGRESS (light — r279 had the journey)
- Provisional heartbeat. Fresh container (pg_hba trust per r205 —
  method-column awk; restore-as-postgres + per-object ALTER owners +
  schema grant per r249). Regression: run-smoke.sh GREEN first pass
  (42 checks, 0 failures, fresh DB + FRESH_BUILD=1; no cold-build
  flake). Two-bot round 280 pending — triage to follow.
