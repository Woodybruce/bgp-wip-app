// r600 journey harness: Landsec client (Mark Warne) on a phone at 390px.
// Task framing: "A tenant rep rang about Bluewater. On the phone, before the
// meeting: find the property in my portfolio, see what's happening on it,
// open the brand behind the enquiry, then log a task so I don't forget."
...
const TAG = process.env.QA_TAG || 'r600';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r600-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
