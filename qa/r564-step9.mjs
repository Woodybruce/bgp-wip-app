...
try {
  ...
  let step=80; const shot=async(l)=>{step++;const p=`qa/smoke-shots/${TAG}-${step}-${l}.png`;await page.screenshot({path:p});console.log('   shot',p);};
  ...
  console.log('== bell badge:', (await page.locator('[data-testid="button-notifications"]').innerText().catch(()=>'')).replace(/\s+/g,' '));
  await page.locator('[data-testid="button-notifications"]').click();
  await page.waitForTimeout(2500);
  await shot('notifications');
  const panel = await page.evaluate(()=>{
    const cands=[...document.querySelectorAll('[role="dialog"],[data-radix-popper-content-wrapper],[data-testid*="notification"]')];
    return cands.map(e=>({id:e.getAttribute('data-testid'),txt:(e.innerText||'').replace(/\s+/g,' ').slice(0,1200)}));
  });
  console.log('== panel:', JSON.stringify(panel).slice(0,2000));
  console.log('== notification testids:', JSON.stringify(await page.evaluate(()=>[...document.querySelectorAll('[data-testid]')].map(e=>e.getAttribute('data-testid')).filter(t=>/notif/i.test(t)).slice(0,40))));
  flush('bell');
