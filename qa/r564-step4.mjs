...
try {
  ...
  let step=30; const shot=async(l)=>{step++;const p=`qa/smoke-shots/${TAG}-${step}-${l}.png`;await page.screenshot({path:p});console.log('   shot',p);};
  ...
  await shot('tasks');
  console.log('== text:', (await page.locator('body').innerText()).replace(/\s+/g,' ').slice(600,3200));
