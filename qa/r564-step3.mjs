...
try {
  ...
  let step=20; const shot=async(l)=>{step++;const p=`qa/smoke-shots/${TAG}-${step}-${l}.png`;await page.screenshot({path:p});console.log('   shot',p);};
  ...
  const card = page.locator('[data-testid="card-landlord-d25ec158-82df-4f50-8188-cae113af5f9f"]');
  console.log('== card:', await card.count(), (await card.innerText().catch(()=>'')).replace(/\s+/g,' ').slice(0,300));
  // find the Open button inside
  const links = await card.evaluate((el)=>[...el.querySelectorAll('button,a')].map(b=>({t:(b.innerText||'').trim().slice(0,30), id:b.getAttribute('data-testid'), href:b.getAttribute('href')})));
  console.log('== card controls:', JSON.stringify(links));
  await card.getByText('Open', {exact:false}).first().click().catch(async(e)=>{console.log('open click failed',String(e).slice(0,120)); await card.click();});
  await page.waitForTimeout(3500); await page.waitForLoadState('networkidle').catch(()=>{});
  console.log('== url after open:', page.url());
  await shot('company');
  console.log('== text:', (await page.locator('body').innerText()).replace(/\s+/g,' ').slice(0,2600));
  console.log('== testids:', JSON.stringify(await page.evaluate(()=>[...new Set([...document.querySelectorAll('[data-testid]')].map(e=>e.getAttribute('data-testid')))].filter(t=>!/^nav-|^chip-threads|^button-panel|^button-pinned/.test(t)).slice(0,140))));
  flush('company');
