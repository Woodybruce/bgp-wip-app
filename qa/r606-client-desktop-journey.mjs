// r606 journey harness: Landsec client (Mark Warne) on the desktop at 1440px.
// Task framing: "Thursday asset-management meeting. I need the void position
// across my centres: what's empty, what's under offer, what's expiring — from
// the dashboard tile through the letting tracker into a centre's schedules,
// and I want a task on the BGP team before I close the laptop."
...
const TAG = process.env.QA_TAG || 'r606';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r606-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
