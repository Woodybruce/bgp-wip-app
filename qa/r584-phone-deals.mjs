// r584c journey harness: Landsec client (Mark Warne) on a phone at 390px.
...
const TAG = process.env.QA_TAG || 'r584c';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r584c-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
...
// Mark taps the Deals tab from the bottom nav: what does BGP say is happening?
await go('/deals', 'deals-cold', {});
for (const w of [3000, 5000, 8000]) {
  await page.waitForTimeout(w);
  const t = await page.evaluate(() => (document.body.innerText || '').replace(/\n{2,}/g,'\n').trim());
  console.log(`\n--- after +${w}ms (${t.length} chars) ---\n` + t.slice(0, 1500));
}
await shot('deals-settled');
for (const tab of ['toggle-deals-properties', 'toggle-deals-deals', 'toggle-deals-letting']) {
  await tap(`[data-testid="${tab}"]`, tab, {});
