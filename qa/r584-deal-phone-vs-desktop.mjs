// r584d journey harness: Landsec client (Mark Warne) on a phone at 390px.
...
const TAG = process.env.QA_TAG || 'r584d';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r584d-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
...
// Mark taps a deal on the phone to see where it's up to; then the SAME deal
// is rendered at 1440px in a desktop context and the two are diffed.
...
console.log('CLIENT DEALS: ' + JSON.stringify(list.map(d => ({ id: d.id, name: d.dealName || d.name, status: d.status }))));
const target = list[0];
if (!target) { console.log('no deals'); process.exit(0); }

await go(`/deals/${target.id}`, 'phone-deal', { text: true, full: true, ids: true });

// same record, desktop context
...
await dp.goto(BASE).catch(() => {});
await dp.evaluate(([tok, u]) => {
  localStorage.setItem('bgp_auth_token', tok); localStorage.setItem('authToken', tok);
  localStorage.setItem('user', JSON.stringify(u));
}, [user.token, user]);
await dp.goto(`${BASE}/deals/${target.id}`).catch(() => {});
await dp.waitForLoadState('networkidle').catch(() => {});
await dp.waitForTimeout(3500);
const dtxt = await dp.evaluate(() => (document.body.innerText || '').replace(/\n{2,}/g,'\n').trim());
await dp.screenshot({ path: 'qa/smoke-shots/r584d-desktop-deal.png' });
console.log('\n=== DESKTOP 1440 SAME DEAL (' + dtxt.length + ' chars) ===\n' + dtxt);
await browser.close();
