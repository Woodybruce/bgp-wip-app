// r584f journey harness: Landsec client (Mark Warne) on a phone at 390px.
...
const TAG = process.env.QA_TAG || 'r584f';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r584f-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
...
const ID = process.env.QA_DEAL || '11110000-0000-0000-0000-000000000302';
async function readIn(context, label, w) {
  ...
  await p.evaluate(([tok, u]) => { localStorage.setItem('bgp_auth_token', tok); localStorage.setItem('authToken', tok); localStorage.setItem('user', JSON.stringify(u)); }, [user.token, user]);
  await p.goto(`${BASE}/deals/${ID}`).catch(()=>{});
  await p.waitForLoadState('networkidle').catch(()=>{});
  await p.waitForTimeout(5000);
  const t = await p.evaluate(() => (document.body.innerText||'').replace(/\n{2,}/g,'\n').trim());
  const ids = await p.evaluate(() => [...new Set([...document.querySelectorAll('[data-testid]')].map(e=>e.getAttribute('data-testid')))]);
  await p.screenshot({ path: `qa/smoke-shots/r584f-${label}.png` });
  console.log(`\n===== ${label} (${w}px) ${t.length} chars =====\n${t}\n--- IDS: ${ids.join(' ')}`);
  return { t, ids };
}
const phone = await readIn(ctx, 'phone', 390);
const dctx = await browser.newContext({ viewport: { width: 1440, height: 1400 } });
const desk = await readIn(dctx, 'desktop', 1440);
const missing = desk.ids.filter(i => !phone.ids.includes(i));
console.log('\n=== testids on DESKTOP but NOT on PHONE ===\n' + missing.join('\n'));
await browser.close();
