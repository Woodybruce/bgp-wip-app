// r584e journey harness: Landsec client (Mark Warne) on a phone at 390px.
...
const TAG = process.env.QA_TAG || 'r584e';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r584e-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
...
const ID = '11110000-0000-0000-0000-000000000301';
console.log('--- goto /deals/' + ID);
await page.goto(`${BASE}/deals/${ID}`).catch(e => console.log('goto err ' + String(e).slice(0,120)));
for (const w of [1500, 2500, 4000]) {
  await page.waitForTimeout(w);
  const st = await page.evaluate(() => ({ p: location.pathname, len: document.body.innerText.length, head: (document.body.innerText||'').slice(0,120) }));
  console.log(`+${w}ms path=${st.p} chars=${st.len} :: ${st.head.replace(/\n/g,' | ')}`);
}
await shot('deeplink-final');
// and via an in-app tap from the Deals list
console.log('\n--- in-app: /deals/list then tap a card');
await page.goto(`${BASE}/deals/list`).catch(()=>{});
await page.waitForTimeout(6000);
const cards = await page.evaluate(() => [...document.querySelectorAll('[data-testid]')].map(e=>e.getAttribute('data-testid')).filter(t=>/deal|card|row/i.test(t)).slice(0,25));
console.log('CARD IDS: ' + JSON.stringify(cards));
await page.evaluate(() => { const el=[...document.querySelectorAll('*')].find(e=>e.textContent?.trim()==="Bluewater MSU9 letting"); if(el) el.closest('[data-testid]')?.click?.() || el.click(); });
await page.waitForTimeout(4000);
const after = await page.evaluate(() => ({ p: location.pathname, len: document.body.innerText.length, head: (document.body.innerText||'').slice(0,300) }));
console.log('AFTER TAP path=' + after.p + ' chars=' + after.len + ' :: ' + after.head.replace(/\n/g,' | '));
