// r560 journey harness: Landsec client (Mark Warne) on a real iPhone context, 390px.
// Task framing: "Monday, walking into a Landsec asset review. Off my phone:
// what has BGP got outstanding on my portfolio — my tasks and my live deals —
// and does every number agree with the list behind it?"
...
const TAG = process.env.QA_TAG || 'r560';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r560-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
...
let step = Number(process.env.QA_STEP0 || 0);
