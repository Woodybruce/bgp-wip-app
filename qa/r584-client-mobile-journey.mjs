// r584 journey harness: Landsec client (Mark Warne) on a phone at 390px.
...
const TAG = process.env.QA_TAG || 'r584';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r584-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
...
// ── JOURNEY (r584) ──────────────────────────────────────────────────────
// Mark Warne, on the train to Bluewater, on his iPhone:
//  "What's happening on my empty units, and let me put a note in for BGP
//   before I forget it — and check it's still there when I look again."
