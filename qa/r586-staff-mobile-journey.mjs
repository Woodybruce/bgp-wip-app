// r586 journey harness: BGP staff (Victoria) on her phone at 390px.
// Task framing: "Just out of a viewing. Log what happened on the unit and
// chase the next step — all on the phone, out of the office."
...
const TAG = process.env.QA_TAG || 'r586';
...
const CACHE = process.env.QA_TOKEN_CACHE || '/tmp/r586-token.json';
let user = null;
if (existsSync(CACHE)) {
  try {
...
// ── JOURNEY (r586) staff phone ──
const PROP = 'cccccccc-0000-0000-0000-000000000001';
await go(`/properties/${PROP}`, 'property');
for (const tab of ['ACTIVITY', 'DEALS & UNITS']) {
  const info = await tap(`text="${tab}"`, `tab-${tab.replace(/\W+/g,'-')}`, {});
  if (info) console.log('   TXT: ' + info.txt.replace(/\n/g, ' | ').slice(0, 1600));
}
await browser.close();
