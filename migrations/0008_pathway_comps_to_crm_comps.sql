...
-- to crm_comps instead, tagged with source_evidence = 'Pathway' and a
-- dedupe key in source_title.
--
...
-- Idempotent: dedupes by the same key stored in source_title.
...
SELECT
  ...
  'pathway-key:' || rlc.dedupe_key                                   AS source_title,
  NULLIF(
    CONCAT_WS(' · ',
      rlc.notes,
...
LEFT JOIN crm_comps cc
  ON cc.source_title = 'pathway-key:' || rlc.dedupe_key
